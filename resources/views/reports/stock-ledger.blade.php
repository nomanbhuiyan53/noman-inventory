...
@section('breadcrumbs')
    ...
    <span class="text-slate-400">/</span>
    <span class="text-slate-900">Stock Ledger</span>
@stop
...
<div class="space-y-6">
    <header class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900">Stock Ledger</h1>
            <p class="mt-1 text-sm text-slate-600">Movement history by document.</p>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Document</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                    @forelse($results ?? [] as $r)
                        <tr class="hover:bg-slate-50">
                            <td class="px-4 py-3 text-sm text-slate-600">{{ $r->postedAt ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm text-slate-900">{{ $r->itemCode ?? $r->itemId ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm text-slate-600">{{ $r->movementType ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm text-slate-600 text-right">{{ $r->quantity ?? 0 }}</td>
                            <td class="px-4 py-3 text-sm text-slate-600">{{ $r->documentNumber ?? '—' }}</td>
                        </tr>
                    @empty
