...
@section('breadcrumbs')
    ...
    <span class="text-slate-900">Issue Stock</span>
@stop

@section('content')
<div class="space-y-6">
    <header>
        <h1 class="text-2xl font-semibold text-slate-900">Issue Stock</h1>
        <p class="mt-1 text-sm text-slate-600">Record stock issued out of a warehouse.</p>
    </header>

    <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        <form action="{{ route('inventory.stock.issue.submit') }}" method="post" class="space-y-4">
            @csrf
            <div>
            ...
            <div>
                <label for="movement_type" class="block text-sm font-medium text-slate-700 mb-1">Movement type *</label>
                <select name="movement_type" id="movement_type" required class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
            ...
            <div class="flex flex-wrap gap-2 pt-2">
                <button type="submit" class="inline-flex items-center px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700">Issue Stock</button>
